[**@xgsd/workers**](README.md)

***

# @xgsd/workers

## Enumerations

- [WorkerErrorCode](enumerations/WorkerErrorCode.md)

## Type Aliases

- [Activation](type-aliases/Activation.md)
- [ActivationHandler](type-aliases/ActivationHandler.md)
- [ActivationSignal](type-aliases/ActivationSignal.md)
- [Context](type-aliases/Context.md)
- [ContextMetadata](type-aliases/ContextMetadata.md)
- [ErrorSignal](type-aliases/ErrorSignal.md)
- [GenericSignal](type-aliases/GenericSignal.md)
- [LogSignal](type-aliases/LogSignal.md)
- [MetricSignal](type-aliases/MetricSignal.md)
- [Signal](type-aliases/Signal.md)
- [SignalType](type-aliases/SignalType.md)
- [SystemSignal](type-aliases/SystemSignal.md)
- [WorkerConfig](type-aliases/WorkerConfig.md)
- [WorkerConfigCache](type-aliases/WorkerConfigCache.md)
- [WorkerConfigCacheStrategy](type-aliases/WorkerConfigCacheStrategy.md)
- [WorkerError](type-aliases/WorkerError.md)
- [WorkerErrorType](type-aliases/WorkerErrorType.md)
- [WorkerOutputMode](type-aliases/WorkerOutputMode.md)
- [WorkerResult](type-aliases/WorkerResult.md)

## Variables

- [version](variables/version.md)

## Functions

- [createHandler](functions/createHandler.md)
- [runWorker](functions/runWorker.md)
